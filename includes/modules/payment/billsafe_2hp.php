...
/* -----------------------------------------------------------------------------------------
  $Id: billsafe_2hp.php 420 2013-06-19 18:04:39Z akausch $
...
class billsafe_2hp {
    ...
    function billsafe_2hp() {
        global $order;
        require (DIR_FS_CATALOG . 'includes/classes/billsafe_2/ini.php'); //DokuMan - 2012-06-19 - move billsafe to external directory
        ...
        if ((int) MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID > 0)
            $this->order_status = MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID;
        if (isset($_SESSION['billsafe_status']) && $_SESSION['billsafe_status'] == 'declined')
            $this->enabled = false; //Dokuman - 2012-06-19 - added isset check
        $this->check();
        if (is_object($order))
            $this->update_status();
    }
    ...
    function selection() {
        ...
        require_once (DIR_FS_CATALOG . 'includes/classes/billsafe_2/billsafe_2.php'); //DokuMan - 2012-06-19 - move billsafe to external directory
        $bs = new Billsafe_Sdk(DIR_FS_CATALOG . 'includes/classes/billsafe_2/ini.php'); //DokuMan - 2012-06-19 - move billsafe to external directory
        if (MODULE_PAYMENT_BILLSAFE_2HP_LOG == 'True') {
            if (MODULE_PAYMENT_BILLSAFE_2HP_LOG_TYPE == 'Echo') {
                require_once DIR_FS_CATALOG . 'includes/classes/billsafe_2/LoggerEcho.php'; //DokuMan - 2012-06-19 - move billsafe to external directory
                $bs->setLogger(new Billsafe_LoggerEcho());
            } elseif (MODULE_PAYMENT_BILLSAFE_2HP_LOG_TYPE == 'Mail') {
                require_once DIR_FS_CATALOG . 'includes/classes/billsafe_2/LoggerMail.php'; //DokuMan - 2012-06-19 - move billsafe to external directory
                ...
                require_once DIR_FS_CATALOG . 'includes/classes/billsafe_2/LoggerFile.php'; //DokuMan - 2012-06-19 - move billsafe to external directory
...
//      $_SESSION['bs_article'] = $article;
    ...
    function install() {
        ...
        if (xtc_db_num_rows($check_query) == 0) {
            ...
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD pause INT NOT NULL DEFAULT "0"');
            $check_query = xtc_db_query('SHOW COLUMNS FROM billsafe_orders_details_2 like "product_id"');
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD product_id VARCHAR(64) NOT NULL AFTER ordernumber');
            $check_query = xtc_db_query('SHOW COLUMNS FROM billsafe_orders_details_2 like "articletype"');
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD articletype VARCHAR(64) NOT NULL AFTER product_id');
            $check_query = xtc_db_query('SHOW COLUMNS FROM billsafe_orders_details_2 like "articlename"');
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD articlename VARCHAR(255) NOT NULL AFTER articlenumber');
            $check_query = xtc_db_query('SHOW COLUMNS FROM billsafe_orders_details_2 like "articleprice"');
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD articleprice VARCHAR(64) NOT NULL AFTER articlename');
            $check_query = xtc_db_query('SHOW COLUMNS FROM billsafe_orders_details_2 like "articletax"');
            if (xtc_db_num_rows($check_query) == 0)
                xtc_db_query('ALTER TABLE billsafe_orders_details_2 ADD articletax VARCHAR(64) NOT NULL AFTER articleprice');
        }
        ...
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_STATUS", "False", "6", "1", "xtc_cfg_select_option(array(\'True\', \'False\'), ", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_LAYER"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_LAYER", "True", "6", "1", "xtc_cfg_select_option(array(\'True\', \'False\'), ", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_LOG"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_LOG", "False", "6", "1", "xtc_cfg_select_option(array(\'True\', \'False\'), ", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_LOG_TYPE"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_LOG_TYPE", "False", "6", "1", "xtc_cfg_select_option(array(\'Echo\', \'Mail\', \'File\'), ", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_LOG_ADDR"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_LOG_ADDR", "", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_ID"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_ID", "", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MERCHANT_LICENSE", "", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_MIN_ORDER"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MIN_ORDER", "0", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_MAX_ORDER"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_MAX_ORDER", "500", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_SHOP_LOGO_URL"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_SHOP_LOGO_URL", "' . $logo_url . '", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_BILLSAFE_LOGO_URL"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_BILLSAFE_LOGO_URL", "' . $billsafe_logo . '", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_SERVER"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_SERVER", "Sandbox", "6", "1", "xtc_cfg_select_option(array(\'Live\', \'Sandbox\'), ", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_ZONE"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ZONE", "0", "6", "2", "xtc_get_zone_class_title", "xtc_cfg_pull_down_zone_classes(", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ORDER_STATUS_ID", "0", "6", "0", "xtc_cfg_pull_down_order_statuses(", "xtc_get_order_status_name", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_SORT_ORDER"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_SORT_ORDER", "0", "6", "0", now())');
        $check_query = xtc_db_query('SHOW COLUMNS FROM ' . TABLE_CONFIGURATION . ' like "MODULE_PAYMENT_BILLSAFE_2HP_ALLOWED"');
        if (xtc_db_num_rows($check_query) == 0)
            xtc_db_query('INSERT INTO ' . TABLE_CONFIGURATION . ' (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ("MODULE_PAYMENT_BILLSAFE_2HP_ALLOWED", "DE", "6", "0", now())');
    }
