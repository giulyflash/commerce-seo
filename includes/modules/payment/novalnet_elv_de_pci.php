...
class novalnet_elv_de_pci {
    ...
    function novalnet_elv_de_pci() {
        ...
        $this->logo_title = MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_LOGO_TITLE;
        $this->payment_logo_title = MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PAYMENT_LOGO_TITLE;
        $this->title = MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_TITLE . '<br>' . $this->logo_title . $this->payment_logo_title;
        ...
        $this->nn_elv_de_pci_image = ( MODULE_PAYMENT_ENABLE_NOVALNET_LOGO == 1) ? $this->logo_title . $this->payment_logo_title : $this->payment_logo_title;
        $this->proxy = MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PROXY;
        $this->password = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PASSWORD);
        ...
        if ($_REQUEST['hash2'] && $_SESSION['payment'] == $this->code) {
            if (strtoupper($this->implementation) == 'JAVA') {#Java encoded
                if ($_REQUEST['auth_code'] != md5(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_AUTH_CODE . strrev($this->password))) {
                    $err = MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_HASH_ERROR . '; wrong auth_code!';
                    $payment_error_return = 'payment_error=novalnet_elv_de_pci&error=' . $_REQUEST['status_text'] . (isset($err) ? ';' . utf8_encode($err) : '');
                    if ($this->is_ajax) {
    ...
    function doAssignConfigVarsToMembers() {
        $this->vendorid = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_VENDOR_ID);
        $this->productid = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PRODUCT_ID);
        $this->authcode = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_AUTH_CODE);
        $this->tariffid = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TARIFF_ID);
        $this->testmode = (strtolower(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEST_MODE == '1') ? 1 : 0;
        $this->password = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PASSWORD);
        $this->manual_check_limit = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_MANUAL_CHECK_LIMIT);
        $this->productid_2 = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PRODUCT_ID2);
        $this->tariffid_2 = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TARIFF_ID2);
    ...
    function checkConfigure() {
        if (IS_ADMIN_FLAG == true) {
            if ($this->enabled == 'true' && (empty($this->vendorid) || empty($this->productid) || empty($this->authcode) || empty($this->tariffid) || empty($this->password))) {
                $this->title .= '<br>' . MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_NOT_CONFIGURED;
            } elseif ($this->testmode == '1') {
                $this->title .= '<br>' . MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_IN_TEST_MODE;
            }
        }
    }
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_ZONE > 0)) {
            $check_flag = false;
            $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
        ...
        $selection = array('id' => $this->code,
            ...
            'fields' => array(
                array('title' => '', 'field' => $this->nn_elv_de_pci_image . $this->description),
                array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_INFO),
    ...
    function pre_confirmation_check($vars) {
        ...
        if (!$order->info['shipping_method']) {
            $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(utf8_encode(MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_REQUEST_FOR_CHOOSE_SHIPPING_METHOD));
            $_SESSION['checkout_payment_error'] = $payment_error_return;
            return;
        }
    ...
    function confirmation() {
        ...
        $_SESSION['nn_total_elv_de_pci'] = $total;
    ...
    function process_button() {
        ...
        if (strtoupper($this->implementation) == 'JAVA') {
            ...
            $hash = md5($auth_code . $product_id . $tariff_id . $amount . $test_mode . $uniqid . strrev($this->password));
            $auth_code = md5($auth_code . strrev($this->password));
        ...
        $process_button_string = xtc_draw_hidden_field('vendor_id', $this->vendorid) .
                ...
                xtc_draw_hidden_field('lang', MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_LANG) .
                xtc_draw_hidden_field('language', MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_LANG) .
    ...
    function before_process() {
        ...
        if ($_POST['status'] == 100) {
            ...
            if (strtoupper($this->implementation) == 'JAVA') {
				$server_test_mode = $this->encode4java($_REQUEST['test_mode'], 'bindec');
			}else{
				$server_test_mode = $this->decode($_REQUEST['test_mode']);
			}
            //$test_mode = $this->testmode;
            ...
            $newlinebreak = "\n";
            $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TID_MESSAGE . $_POST['tid'] . $newlinebreak;
    ...
    function after_process() {
        ...
        if (isset($_SESSION['nn_tid_elv_de_pci'])) {
            ...
            $urlparam .= '&auth_code=' . $this->authcode . '&status=100&tid=' . $_SESSION['nn_tid_elv_de_pci']
                    . '&vwz2=' . MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_ORDERNO . '' . $insert_id .
    ...
    function get_error() {
        ...
        if (count($HTTP_GET_VARS) == 0 || $HTTP_GET_VARS == '')
            $HTTP_GET_VARS = $_GET;
        $error = array('title' => MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEXT_ERROR, 'error' => stripslashes(html_entity_decode($HTTP_GET_VARS['error'])));
        return $error;
    }
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_STATUS'");
    ...
    function install() {
        ...
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_ALLOWED', '', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_STATUS', 'True', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TEST_MODE', 'True', '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_VENDOR_ID', '', '6', '3', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_AUTH_CODE', '', '6', '4', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PRODUCT_ID', '', '6', '5', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TARIFF_ID', '', '6', '6', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_MANUAL_CHECK_LIMIT', '', '6', '7', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PRODUCT_ID2', '', '6', '8', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_TARIFF_ID2', '', '6', '9', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PASSWORD', '', '6', '10', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_INFO', '', '6', '11', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_SORT_ORDER', '0', '6', '12', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_ORDER_STATUS_ID', '0', '6', '13', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_ZONE', '0', '6', '14', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_NOVALNET_ELV_DE_PCI_PROXY', '', '6', '15', now())");
    }
    ...
    function encode($data) {
        ...
        try {
            ...
            $data = $crc . "|" . $data;
            $data = bin2hex($data . $this->password);
    ...
    function decode($data) {
        ...
        try {
            ...
            $data = substr($data, 0, stripos($data, $this->password));
            $pos = strpos($data, "|");
            if ($pos === false) {
    ...
    function hash($h) {
        ...
        return md5($h['auth_code'] . $h['product_id'] . $h['tariff'] . $h['amount'] . $h['test_mode'] . $h['uniqid'] . strrev($this->password));
    }
