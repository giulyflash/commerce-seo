      ...
      for ($i = 0, $n = sizeof($include_modules); $i < $n; $i++) {
        if (!in_array($include_modules[$i]['class'], $unallowed_modules)) {
          ...
          if (defined(MODULE_PAYMENT_ . strtoupper(str_replace('.php', '', $include_modules[$i]['file'])) . _ZONE)) {
            $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . constant(MODULE_PAYMENT_ . strtoupper(str_replace('.php', '', $include_modules[$i]['file'])) . _ZONE) . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
