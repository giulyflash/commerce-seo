...
/* -----------------------------------------------------------------
 * 	$Id: cseo_image_processing_info.php 420 2013-06-19 18:04:39Z akausch $
...
define('MODULE_INFO_IMAGE_PROCESS_TEXT_DESCRIPTION', 'Es werden alle Bilder in den Verzeichnissen<br /><br />
...
define('MODULE_INFO_IMAGE_PROCESS_TEXT_TITLE', 'CSEO-Imageprocessing-Info <b>-V2.2 (info_images)</b>');
define('MODULE_INFO_IMAGE_PROCESS_STATUS_DESC', 'Modulstatus');
define('MODULE_INFO_IMAGE_PROCESS_STATUS_TITLE', 'Status');
...
if (!class_exists("cseo_image_processing_info")) {

    class cseo_image_processing_info {

        var $code, $title, $description, $enabled;

        function cseo_image_processing_info() {
            global $order;

            $this->code = 'cseo_image_processing_info';
            $this->title = MODULE_INFO_IMAGE_PROCESS_TEXT_TITLE;
            $this->description = sprintF(MODULE_INFO_IMAGE_PROCESS_TEXT_DESCRIPTION, $_GET['max']);
            $this->sort_order = MODULE_INFO_IMAGE_PROCESS_SORT_ORDER;
            $this->enabled = ((MODULE_INFO_IMAGE_PROCESS_STATUS == 'True') ? true : false);
        }
        ...
        function process($file, $offset) {
            ...
            for ($i = $offset; $i < $limit; $i++) {
                if ($i >= $max_files) { // FERTIG
                    ...
                    xtc_redirect(xtc_href_link(FILENAME_MODULE_SYSTEM, 'set=' . $_GET['set'] . '&module=cseo_image_processing_info&infotext=' . $infotext . '&max=' . $_GET['max'])); //FERTIG
                }
        ...
        function display() {
            ...
            return array('text' =>
                ...
                xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_SYSTEM, 'set=' . $_GET['set'] . '&module=cseo_image_processing_info'))
            );
        }

        function check() {
            if (!isset($this->_check)) {
                $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_INFO_IMAGE_PROCESS_STATUS'");
        ...
        function install() {
            xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_INFO_IMAGE_PROCESS_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        }
        ...
        function keys() {
            return array('MODULE_INFO_IMAGE_PROCESS_STATUS');
        }
