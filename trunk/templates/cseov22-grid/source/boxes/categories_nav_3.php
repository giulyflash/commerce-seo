...
/* -----------------------------------------------------------------
 *  $Id: categories_nav_3.php 486 2013-07-15 22:08:14Z akausch $
...
if (!CacheCheck() && !FORCE_CACHE) {
    ...
    $cache_id = $_SESSION['language'] . $_SESSION['customers_status']['customers_status_id'] . $cPath.'headcat3';
}

if (!$box_smarty->isCached(CURRENT_TEMPLATE . '/boxes/box_categories_nav.html', $cache_id) || !$cache) {
    $mintmp = 4;
...
if (!$cache) {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE . '/boxes/box_categories_nav.html');
} else {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE . '/boxes/box_categories_nav.html', $cache_id);
}
