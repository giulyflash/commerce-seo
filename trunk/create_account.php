...
/* -----------------------------------------------------------------
 * 	$Id: create_account.php 471 2013-07-09 18:32:20Z akausch $
...
$smarty->assign('FORM_ACTION', xtc_draw_form('create_account', xtc_href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL'), 'post', 'onsubmit="return check_form(this);" autocomplete="off"') . xtc_draw_hidden_field('action', 'process'));
...
$smarty->assign('INPUT_PASSWORD', xtc_draw_password_fieldNote(array('name' => 'password', 'text' => '&nbsp;' . (xtc_not_null(ENTRY_PASSWORD_TEXT) ? '<span class="inputRequirement">' . ENTRY_PASSWORD_TEXT . '</span>' : '')), '', 'size="29" class="password" id="create_password"'));
...
if (TRUSTED_SHOP_CREATE_ACCOUNT_DS == 'true') {
    ...
    $smarty->assign('DATENSG_CHECKBOX', '<input type="checkbox" value="datensg" name="datensg" id="create_dsg" />');
} else {
