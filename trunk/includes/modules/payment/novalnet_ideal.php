...
class novalnet_ideal {
    ...
    function novalnet_ideal() {
        global $order;
        $this->key = trim(MODULE_PAYMENT_NOVALNET_IDEAL_PASSWORD);
		...
		$this->nn_ideal_image = ( MODULE_PAYMENT_ENABLE_NOVALNET_LOGO ==1) ? $this->logo_title.$this->payment_logo_title : $this->payment_logo_title;
        ...
        if ($_REQUEST['hash2'] && $_SESSION['payment'] == $this->code) {
            if (strtoupper($this->implementation) == 'JAVA') {#Java encoded
                if ($_REQUEST['auth_code'] != md5(MODULE_PAYMENT_NOVALNET_IDEAL_AUTH_CODE . strrev($this->key))) {
                    $err = MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_HASH_ERROR . '; wrong auth_code!';
                    $payment_error_return = 'payment_error=novalnet_ideal&error=' . $_REQUEST['status_text'] . (isset($err)?';'.utf8_encode($err):'');
                    if ($this->is_ajax) {
                ...
                $_REQUEST['auth_code'] = MODULE_PAYMENT_NOVALNET_IDEAL_AUTH_CODE; #todo: check?
    ...
    function doAssignConfigVarsToMembers() {
        $this->vendorid = trim(MODULE_PAYMENT_NOVALNET_IDEAL_VENDOR_ID);
        $this->productid = trim(MODULE_PAYMENT_NOVALNET_IDEAL_PRODUCT_ID);
        $this->authcode = trim(MODULE_PAYMENT_NOVALNET_IDEAL_AUTH_CODE);
        $this->tariffid = trim(MODULE_PAYMENT_NOVALNET_IDEAL_TARIFF_ID);
        $this->testmode = (strtolower(MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE == '1') ? 1 : 0;
        $this->password = trim(MODULE_PAYMENT_NOVALNET_IDEAL_PASSWORD);
    }
    ...
    function checkConfigure() {
        if (IS_ADMIN_FLAG == true) {
            if ($this->enabled == 'true' && (empty($this->vendorid) || empty($this->productid) || empty($this->authcode) || empty($this->tariffid) || empty($this->password))) {
                $this->title .= '<br>'.MODULE_PAYMENT_NOVALNET_IDEAL_NOT_CONFIGURED;
            } elseif ($this->testmode == '1') {
                $this->title .= '<br>'.MODULE_PAYMENT_NOVALNET_IDEAL_IN_TEST_MODE;
            }
        }
    }
 ...
 */
    function selection() {
        ...
        $selection = array('id' => $this->code,
            ...
            'fields' => array(
                array('title' => '', 'field' => $this->nn_ideal_image. $this->description),
    ...
    function process_button() {
        ...
        $process_button_string =
                xtc_draw_hidden_field('vendor', $vendor_id) .
                ...
                xtc_draw_hidden_field('lang', MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_LANG) . #default: 'DE'
                xtc_draw_hidden_field('language', MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_LANG) . #default: 'DE'
    ...
    function before_process() {
        ...
        if ($_POST['tid'] && $_POST['status'] == '100') {
            ...
            if ($test_order_status == 1) {
                $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_IDEAL_TEST_ORDER_MESSAGE;
            }
            if (count($HTTP_POST_VARS) == 0 || $HTTP_POST_VARS == '')
                $HTTP_POST_VARS = $_POST;
				$newlinebreak = "\n";
            $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_IDEAL_TID_MESSAGE . $HTTP_POST_VARS['tid'] . $newlinebreak;
            //$order->info['comments']  = str_replace(array('<b>', '</b>','<B>','</B>', '<br>','<br />','<BR>'), array('', '', '','',"\n", "\n","\n"), $order->info['comments']);	
            $order->info['comments'] .= $old_comments;
            $_SESSION['nn_tid_ideal'] = $HTTP_POST_VARS['tid']; #todo: 
        }
        else {
            $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(utf8_decode($_POST['status_text'])) . '(' . $_POST['status'] . ')';
            if ($this->is_ajax) {
                xtc_redirect(xtc_href_link(FILENAME_CHECKOUT, '', 'SSL', true, false).'?'.$payment_error_return);
            } else {
                xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
 ...
 */
    ...
    function after_process() {
        ...
        $product_id = $this->productid;
        $tariff_id = $this->tariffid;
        if (isset($_SESSION['nn_tid_ideal'])) {
            ...
            $urlparam .= '&auth_code=' . $this->authcode . '&status=100&tid=' . $_SESSION['nn_tid_ideal'] . '&vwz2=' . MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_ORDERNO . '' . $insert_id . '&vwz3=' . MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_ORDERDATE . '' . date('Y-m-d H:i:s');
    ...
    function get_error() {
        ...
        if (count($HTTP_GET_VARS) == 0 || $HTTP_GET_VARS == '')
            $HTTP_GET_VARS = $_GET;
        $error = array('title' => MODULE_PAYMENT_NOVALNET_IDEAL_TEXT_ERROR, 'error' => stripslashes(urldecode($HTTP_GET_VARS['error'])));
        return $error;
    }
 ...
 */
    function install() {
        ...
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_NOVALNET_IDEAL_STATUS', 'True', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE', 'True', '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        ...
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_NOVALNET_IDEAL_ORDER_STATUS_ID', '0', '6', '9', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
