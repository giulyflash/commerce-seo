...
/*************************************************************/
/******* fuers express als Zahlbedingung **********************/
/*************************************************************/
	function paypal_auth_call(){
		...
		// Daten mitgeben, da direkt bestaetigung ohne nochmaliges Confirm im Shop
		// Stand: 05.01.2010
...
/*************************************************************/
/******* fuers express aus dem warenkorb **********************/
/*************************************************************/
...
/*************************************************************/
	function create_shipping_address(){
		// Stand: 28.04.2011
		if(!$_SESSION['reshash']['SHIPTOCITY']) return;
...
/*************************************************************/
/******* funktionen fuer beide versionen **********************/
/*************************************************************/
	//  hash_call: Function to perform the API call to PayPal using API signature
	//  @methodName is name of API  method.
	//  @nvpStr is nvp string.
	//  returns an associtive array containing the response from the server.
	//  08.01.2009.ergaenzt fuer PHP ohne cURL von Stefan Kl.
...
/*************************************************************/
	function paypal_get_products($paymentAmount,$order_tax,$order_discount,$order_fee,$order_shipping,$order_gs,$express_call=False){
		// fuer beide PayPal Versionen
		// Artikel Details mitgeben incl. Attribute
		// Fuer den Express Call Vermerk fuer den Versand + Vorlaeufige Kosten mitgeben
		// Stand: 19.10.2010
		global $xtPrice,$order;
				...
				if($ipn_data['num_cart_items']>1):
					...
					for($p=1;$p<=$verspos;$p++):
						if( $this->data['item_name'.$p] == substr(SUB_TITLE_OT_DISCOUNT,0,127)
							...
							// Artikel Nummer aus den Details fuer Sonderzeilen
...
/*************************************************************/
	function mn_iconv($t1,$t2,$string){
		...
		/// Kein iconv im PHP
		if($t2 == "UTF-8"):
			// nur als Ersatz fuer das iconv und nur in eine richtung 1251 to UTF8
			//ISO 8859-1 to UTF-8
